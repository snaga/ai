...
def setnext(s, db, num):
    assert len(s) >= 1 and isinstance(s, unicode)
    pos = setrnd(num)
    idx = 0
...
#        print(node.feature.decode('utf-8'))
        if node.feature.decode('utf-8').split(',')[0] in (u'BOS/EOS'):
            node = node.next
            continue
        aa.append(u'%s' % node.surface.decode('utf-8'))
        node = node.next
    for a in aa:
...
def test():
    ...
    assert findch(u'小', db) == 3
    assert findch(u'小学', db) == 1

    random.seed(1)
    assert setnext(u'小', db, 3) == u'供'

    random.seed(10)
    assert setnext(u'小', db, 3) == u'に'

    random.seed(1)
    assert setrndstr(db) == u'の'
    assert setrndstr(db) == u'抜かす'
    assert setrndstr(db) == u'大きな'
